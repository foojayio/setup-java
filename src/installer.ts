...
async function getDownloadInfo(
  ...
  if (distro) {
    if (distro === '') {
      ...
      throw new Error(
        `distro argument '${distro}' is not in [aoj | aoj_openj9 | corretto | dragonwell | liberica | microsoft | ojdk_build | openlogic | oracle_openjdk | sap_machine | semeru | temurin | trava | zulu]`
      );
    }
  } else {
    distribution = 'zulu';
